...
static float SURFACE_WIDTH = 708.0;
static float SURFACE_HEIGHT = 580.0;
...
- (id)initWithFrame:(NSRect)frame {
    ...
    if (self = [super initWithFrame:frame])
	{
		NSLog(@"initWithFrame");
		lastFocus = 0;
		SURFACE_WIDTH = self.bounds.size.width;
		SURFACE_HEIGHT = self.bounds.size.height;
...
- (void)initAllChannel {
	...
	for(i = 0; i < SISTEEN_GRID; i++)
	{
		for(j = 0; j < SISTEEN_GRID; j++)
		{
            ...
            [item resetChannelNum:(unsigned int)cfg->channelNum];
			if(i == 0 && j == 0)
			{
				[item setFocus:YES];
			}
...
- (void)viewDidEndLiveResize {
	...
	curRect = self.bounds;
	SURFACE_WIDTH = curRect.size.width;
	SURFACE_HEIGHT = curRect.size.height;
	[self resetAllChannel];
...
- (void)setFrameSize:(NSSize)newSize {
	...
	NSLog(@"xxxxxxxxxxxxxxxxsetFrameSize");
	curRect = self.bounds;
	SURFACE_WIDTH = curRect.size.width;
	SURFACE_HEIGHT = curRect.size.height;
	[self resetAllChannel];
...
- (void)startRecord {
	...
	if(playback)
	{
		if(reqRecord == NO)
		{
            ...
            sprintf(recPath, "%s/DvrRecord/%s/record", homePath, playback->ip);
            ...
            sprintf(recFile, "%s/ch%02d-%d%02d%02d-%02d%02d%02d.h264", recPath, playback->channel, 
                    (1900+p->tm_year), (1+p->tm_mon), p->tm_mday, (p->tm_hour+8), p->tm_min, p->tm_sec);

            MAC_DVS_SaveRealDataStart(playback->hDevStreamHandle,recFile);
			reqRecord = YES;
		}
	}
...
- (void)stopRecord {
	...
	if(playback)
	{
		if(reqRecord == YES)
		{
            ...
            MAC_DVS_SaveRealDataStop(playback->hDevStreamHandle);
			reqRecord = NO;
		}
	}
...
- (int)createStreamPlaybackWithChannel:(unsigned int)ch {
    ...
    DvrConfig *cfg;
    cfg = kt_dvrMgr_getCurDvr();
    memcpy(playback->ip,cfg->serverAddr,sizeof(playback->ip));
    playback->port = cfg->port;
    playback->channel = ch+1;
...
{
    NSLog(@"reset chaneel number");

	if(playback)
	{
		stream_playback_setChannelNum(playback, chNum);		
	}
...
- (void)displayYUVFrame {
	...
	if(pEntry == NULL)
	{
		if(stream_playback_getNetworkStatus(playback) == 1) /*drop line*/
		{
...
- (int)ptzCruisePreSet:(unsigned int)para {
	if(playback)
	{
		return stream_playback_ptzCruisePreSet(playback, para);
	}
...
- (int)ptzCruiseRecall:(unsigned int)para {
	if(playback)
	{
		return stream_playback_ptzCruiseRecall(playback, para);
	}
...
- (int)ptzCruiseStart:(unsigned int)para {
	if(playback)
	{
		return stream_playback_ptzCruiseStart(playback, para);
	}
...
- (int)ptzCruiseClear:(unsigned int)para {
	if(playback)
	{
		return stream_playback_ptzCruiseClear(playback, para);
	}
...
- (int)ptzCruiseLoad:(unsigned int)para {
	if(playback)
	{
		return stream_playback_ptzCruiseLoad(playback, para);
	}
...
- (int)ptzCruiseSave:(unsigned int)para {
	if(playback)
	{
		return stream_playback_ptzCruiseSave(playback, para);
	}
...
- (int)ptzCruiseStop:(unsigned int)para {
	if(playback)
	{
		return stream_playback_ptzCruiseStop(playback, para);
	}
