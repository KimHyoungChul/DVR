...
static int kt_language_resFiniPriview()
{
	...
	if(previewCruiseSet)
	{
		free(previewCruiseSet);
		previewCruiseSet = NULL;
	}
	if(previewCruiseStart)
	{
		free(previewCruiseStart);
		previewCruiseStart = NULL;
	}
	if(previewCruiseStop)
	{
		free(previewCruiseStop);
		previewCruiseStop = NULL;
	}
	if(previewCruiseRecall)
	{
		free(previewCruiseRecall);
		previewCruiseRecall = NULL;
	}
	if(previewCruiseLoad)
	{
		free(previewCruiseLoad);
		previewCruiseLoad = NULL;
	}
	if(previewCruiseSave)
	{
		free(previewCruiseSave);
		previewCruiseSave = NULL;
	}
	if(previewCruiseClear)
	{
		free(previewCruiseClear);
		previewCruiseClear = NULL;
	}
...
static int kt_language_resFiniSettingManage()
{
	...
	{
		free(settingManageDeviceID);
		settingManageDeviceID = NULL;
	}
...
static int kt_language_resFiniGolbal()
{
	...
	{
		free(golbalSimpleChannel);
		golbalSimpleChannel = NULL;
	}
	if(golbalSize)
	{
		free(golbalSize);
...
int kt_language_previewDump(const TiXmlElement &element)
{
	...
	while(child)
	{
		...
		else if(strcmp(string, "IDC_BTN_CRUISESET") == 0)
		{
			string = (char*)child->Attribute("_name_");
			printf("Btn Cruise Set: %s \n", string);
			previewCruiseSet = m_remalloc(previewCruiseSet, strlen(string));
			strcpy(previewCruiseSet, string);
		}
		else if(strcmp(string, "IDC_BTN_CRUISESTART") == 0)
		{
			string = (char*)child->Attribute("_name_");
			printf("Btn Cruise Set: %s \n", string);
			previewCruiseStart = m_remalloc(previewCruiseStart, strlen(string));
			strcpy(previewCruiseStart, string);
		}
		else if(strcmp(string, "IDC_BTN_CRUISESTOP") == 0)
		{
			string = (char*)child->Attribute("_name_");
			printf("Btn Cruise Set: %s \n", string);
			previewCruiseStop = m_remalloc(previewCruiseStop, strlen(string));
			strcpy(previewCruiseStop, string);
		}
		else if(strcmp(string, "IDC_BTN_CRUISERECALL") == 0)
		{
			string = (char*)child->Attribute("_name_");
			printf("Btn Cruise Set: %s \n", string);
			previewCruiseRecall = m_remalloc(previewCruiseRecall, strlen(string));
			strcpy(previewCruiseRecall, string);
		}
		else if(strcmp(string, "IDC_BTN_CRUISELOAD") == 0)
		{
			string = (char*)child->Attribute("_name_");
			printf("Btn Cruise Set: %s \n", string);
			previewCruiseLoad = m_remalloc(previewCruiseLoad, strlen(string));
			strcpy(previewCruiseLoad, string);
		}
		else if(strcmp(string, "IDC_BTN_CRUISESAVE") == 0)
		{
			string = (char*)child->Attribute("_name_");
			printf("Btn Cruise Set: %s \n", string);
			previewCruiseSave = m_remalloc(previewCruiseSave, strlen(string));
			strcpy(previewCruiseSave, string);
		}
		else if(strcmp(string, "IDC_BTN_CRUISECLEAR") == 0)
		{
			string = (char*)child->Attribute("_name_");
			printf("Btn Cruise Set: %s \n", string);
			previewCruiseClear = m_remalloc(previewCruiseClear, strlen(string));
			strcpy(previewCruiseClear, string);
		}
...
int kt_language_settingManageDump(const TiXmlElement &element)
{
	...
	while(child)
	{
		...
		else if(strcmp(string, "IDC_LABEL_DEVICEID") == 0)
		{
			string = (char*)child->Attribute("_name_");
			printf("Status name: %s\n", string);
			settingManageDeviceID= m_remalloc(settingManageDeviceID, strlen(string));
			strcpy(settingManageDeviceID, string);
		}
		child = child->NextSiblingElement();
	}
...
int kt_language_golbalDump(const TiXmlElement &element)
{
	...
	while(child)
	{
		...
		else if(strcmp(string, "IDC_SIMPLE_CHANNEL") == 0)
		{
			string = (char*)child->Attribute("_name_");
			printf("Info name: %s\n", string);
			golbalSimpleChannel = m_remalloc(golbalSimpleChannel, strlen(string));
			strcpy(golbalSimpleChannel, string);
		}
		...
		else if(strcmp(string, "IDC_DOWNLOAD_FINISH") == 0)
		{
			string = (char*)child->Attribute("_name_");
			printf("Info name: %s\n", string);
			golbalDownloadFinish = m_remalloc(golbalDownloadFinish, strlen(string));
			strcpy(golbalDownloadFinish, string);
		}
		child = child->NextSiblingElement();
	}
