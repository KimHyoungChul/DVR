...
UNSL32  RemoteFilePlayer(LPVOID lpvoid);
...
CRemoteFilePlayer::CRemoteFilePlayer()
{
    ...
    m_hStepEvent.CreateUnsignaledEvent("m_hStepEvent",1);
	...
	while(1)
	{
		...
		if (AUDIO_FRAME == avType)
		{
			...
			if (player->m_cbRemoteData.cbRemoteFunc)
			{
                ...
                player->m_cbRemoteData.cbRemoteFunc((void*)player->m_cbRemoteData.dwUser, curMP4Buf, curMp4peslen, 0, 1);
			}
		}
		...
		if (VIDEO_FRAME == avType && player->m_bFindIFrame[chID])
		{
			...
			if (player->m_cbRemoteData.cbRemoteFunc)
			{
                ...
                Stream_Playback *inst = (Stream_Playback*)(player->m_cbRemoteData.dwUser);
                player->m_cbRemoteData.cbRemoteFunc((void*)player->m_cbRemoteData.dwUser, curMP4Buf, curMp4peslen, 0, 1);
			}
			...
			{	
				pts_cur[chID] = player->m_RemotePSFile.GetPTS();
				...
				while(1)
				{
					if (player->m_bNeedReset[chID])
					{
						...
						pts_last[chID] = player->m_RemotePSFile.GetPTS();
...
void CRemoteFilePlayer::SetRemoteDataCallBack(process_cb_ex cbRemoteFunc, void* dwUser)
{
	if (cbRemoteFunc)
	{
