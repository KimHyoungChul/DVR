...
static unsigned int m_up_iphone_countTick(void)
{
    ...
    totalMillisecondsHi += (totalMillisecondsLo < time_val.tv_usec / 1000);
  	...
  	return (unsigned int)(totalMillisecondsLo&0xffffffff);
}
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

UNSL32 LocalFilePlayer(LPVOID lpvoid);
...
CLocalFilePlayer::CLocalFilePlayer()
{
    ...
    m_hStepEvent.CreateUnsignaledEvent("m_hStepEvent", 0);
...
UNSL32 LocalFilePlayer(LPVOID lpvoid)
{
    ...
    //控制播放速度
    unsigned int stc_start[MAX_SOLO_CHANNEL_16] = {0};
    unsigned int stc_current[MAX_SOLO_CHANNEL_16] = {0};
    unsigned int stc_duration[MAX_SOLO_CHANNEL_16] = {0};
    unsigned int pts_last[MAX_SOLO_CHANNEL_16] = {0};
    unsigned int pts_cur[MAX_SOLO_CHANNEL_16] = {0};
    unsigned int pts_duration[MAX_SOLO_CHANNEL_16] = {0};
	...
	while(1)
	{
		...
		//如果此时是倒退模式	
		printf("play->m_PlayDirect != PLAY_BACKWARD\n");
		if (player->m_PlayDirect == PLAY_BACKWARD)
		{
			printf("play->m_PlayDirect ======= PLAY_BACKWARD\n");
		...
		if (player->m_PESPack.GetAVType() == AUDIO_FRAME)
		{
			...
			if (player->m_cbLocalData.cbLocalFunc)
			{
                ...
                player->m_cbLocalData.cbLocalFunc((void*)player->m_cbLocalData.dwUser, curMP4Buf, curMp4peslen, 0, 1);
			}
		}
		...
		if (avType == VIDEO_FRAME && player->m_PESPack.FindIFrame[chID])
		{
			...
			if (player->m_cbLocalData.cbLocalFunc)
			{
                ...
                Stream_Playback *inst = (Stream_Playback*)(player->m_cbLocalData.dwUser);
                player->m_cbLocalData.cbLocalFunc((void*)player->m_cbLocalData.dwUser, curMP4Buf, curMp4peslen, 0, 1);
			}
			...
			{/*
                ...
                player->m_hStepEvent.WaitSemaphore();
			}
            
            //本地视频同步
            if(!(player->m_bStepPlay || player->m_bFastPlay || (player->m_PlayDirect == PLAY_BACKWARD)))
            {
                pts_cur[chID] = player->m_PESPack.GetPTS();
                if(pts_cur[chID] >= pts_last[chID])
                {
                    if((pts_cur[chID] - pts_last[chID]) > 3000)
                    {
                        player->m_bNeedReset[chID] = true;
                    }else
                    {
                        pts_duration[chID] += ((pts_cur[chID] - pts_last[chID]) / player->m_RateScale);
                        pts_last[chID] = pts_cur[chID];
                    }
                }else
                {
                    player->m_bNeedReset[chID] = true;
                ...
                while(1)
				{
					if (player->m_bNeedReset[chID])
					{	                        
    					stc_start[chID] = m_up_iphone_countTick();
						pts_last[chID] = player->m_PESPack.GetPTS();
						pts_cur[chID] = pts_last[chID];
						pts_duration[chID] = 0;
						stc_duration[chID] = 0;
						player->m_bNeedReset[chID] = false;
					}
                    

					stc_current[chID] = m_up_iphone_countTick();
					if (stc_current[chID] >= stc_start[chID])
					{
						stc_duration[chID] += (DWORD)(stc_current[chID] - stc_start[chID]);
						stc_start[chID] = stc_current[chID];
					}else
					{
						stc_duration[chID] += (DWORD)(0xffffffff - stc_start[chID]);
						stc_duration[chID] += (DWORD)(stc_current[chID]);
						stc_start[chID] = stc_current[chID];
					}
                    
					if (stc_duration[chID] >= pts_duration[chID])
					{
						break;
					}
					if (player->m_bExit)
					...
					//该等待是用来控制视频的播放
					//WaitForSingleObject(player->m_hTimerEvent, 5000);
				}
            }
...
void CLocalFilePlayer::SetLocalDataCallBack(process_cb_ex cbLocalFunc, void* dwUser)
{
	if (cbLocalFunc)
	{
