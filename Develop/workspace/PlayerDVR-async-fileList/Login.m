...
-(IBAction)signedIn:(id)sender {
	...
	/*do login*/
	nsStr = [IPAddress stringValue];
	tmpIPAddr = (char*)[nsStr UTF8String];
	tmpUsrName = NULL;
	nsStr = [password stringValue];
	tmpPwd = (char*)[nsStr UTF8String];
	nsStr = [netPort stringValue];
	port = [nsStr intValue];
	[notify setStringValue:@"Login ongoing"];
	loginRet = upui_fw_srvLogin((UPSint8*)tmpIPAddr, port, (UPUint8*)tmpUsrName, (UPUint8*)tmpPwd);
	if(loginRet == 0x0a)
	{
