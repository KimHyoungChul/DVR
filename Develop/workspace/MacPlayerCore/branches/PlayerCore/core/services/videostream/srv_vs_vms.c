						   ...
						   UPUint8 month, UPUint8 day, UPSint type, UPUint *fileNum)
{
	...
	if(respHead->errorcode != 0)
	{
		UPLOG(vs_vms_searchGetNum, ("%x, server response error", respHead->errorcode));
		up_sock_close(socket);
		ucm_free(UP_NULL, respHead);
		return -1;
	}
			...
			UPUint8 month, UPUint8 day, UPSint type, UPSint8 **fileList, UPUint *fileNum)
{
	...
	if(respHead->errorcode != 0)
	{
		UPLOG(vs_vms_searchFile, ("%x, server response error", respHead->errorcode));
		up_sock_close(socket);
		ucm_free(UP_NULL, respHead);
		*fileNum = 0;
		*fileList = UP_NULL;
		return respHead->errorcode;
	}
