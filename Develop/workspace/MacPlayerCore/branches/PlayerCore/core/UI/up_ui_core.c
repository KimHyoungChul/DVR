...
UPSint upui_fw_srvSearchFile(Search_FileMgr *instance, UPSint8 *addr, UPUint32 port, UPUint channel, UPUint8 year, UPUint8 month, UPUint8 day, UPUint8 type)
{
	...
	listTmp = srv_com_vmsGetFileList2(addr, port, channel, year, month, day, type, &fileNum, &result);
	UPLOG(upui_fw_srvSearchFile, ("file number: %d", fileNum));
	
	if(listTmp == UP_NULL)
	{
		if(result == RME_OTHER_TASKGOING)
		{
			UPLOG(upui_fw_srvSearchFile, ("DVR menu popup in native!"));
			ret = DVR_BUSY;
		}
		else if(result == -1)
		{
			UPLOG(upui_fw_srvSearchFile, ("error"));
			ret = -1;
		}
		else if(result == FILE_SEARCH_NOTFOUND)
		{
