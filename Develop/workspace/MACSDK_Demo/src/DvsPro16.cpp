...
UNSL32 __attribute__((stdcall)) DumpFileThread(LPVOID lpvoid);
UNSL32 __attribute__((stdcall)) DumpFileThread(LPVOID lpvoid)
{
...
/////////
//////////////////////////////////////////////////////////////
UNS32 DvsPro16::QueryRecordFile(DVS_TIME dtStartTime, DVS_TIME dtEndTime, UNS8 type, LPDVS_RECORDFILE_LIST lpRecordfileList)
{
	...
	for (UNS32 i=0; i<RecordfileDateInfo.ItemNum; i++)
	{
			...
			CompareDvsTime(dtEndTime, dtTempTime2) <= 0)
		{
			...
			{
				localtime_s(pTime, &DayRecordfileInfo.File[j].start_time);
				if (pTime != NULL)
				{
				...
				localtime_s(pTime, &DayRecordfileInfo.File[j].end_time);
				if (pTime != NULL)
				{
...
bool DvsPro16::GetRemotePlayTime(LPDVS_TIME lpCurPlayTime)
{
	...
	localtime_s(timeTemp, (const time_t*)&curtime);
	if (timeTemp != NULL)
	{
...
void DvsPro16::SetRemoteDataCallBack(fDataCallBack cbRemoteFunc, UNS32 dwUser)
{
	if (cbRemoteFunc)
	{
